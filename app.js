...
visitsRef.on('value', snapshot => {
  ...
  // فتح الصور والفيديوهات بالـ fullscreen
...
<div id="cmp-banner">
  ...
  <button id="consent-allow">أوافق</button>
  <button id="consent-manage">تغيير الخيارات</button>
</div>

<script src="app.js" defer></script>
<script>

// ======= CMP Script + كل الكود الأصلي =======
document.addEventListener('DOMContentLoaded', () => {
...
// كل الصور والفيديوهات اللي تحب تفتح في fullscreen
document.querySelectorAll('.service-card img, .service-card video').forEach(el => {
  el.style.cursor = 'pointer';
  el.addEventListener('click', () => {
    mediaViewer.style.display = 'flex';
...
closeBtn.addEventListener('click', () => {
  ...
  let currentLanguage = 'ar';
 // ======= Visits Counter =======
let totalVisits = 0; // متغير عام للعدد
...
// تحديث العدد مباشرة على الموقع حسب اللغة
visitsRef.on('value', snapshot => {
  totalVisits = snapshot.val() || 0;
  updateVisitText(currentLanguage, totalVisits);
});
...
document.querySelectorAll('.video-card video').forEach(video => {
  ...
  ar: {
    title: "خدمات اليوم",
    captions: [
      "تصليح لوحات إلكترونية",
      "تصليح كارت جهاز المشي",
      "تصليح كارت ماكينة غسيل",
      "تصليح كارت تلفاز",
      "تصليح كارت غسالة ل-ج"
    ]
  },
  fr: {
    ...
    captions: [
      ...
      "réparation carte tapis roulant",
      "réparation carte lave-linge",
      "réparation carte television",
      "réparation carte lave-linge LG"
    ]
  }
};
...
const videoTranslations = {
  ...
  fr: {
    title: "Vidéo du jour",
    captions: [
      "réparation carte alimentation",
      "réparation carte électronique",
      "inspection carte 1",
      "inspection carte 2",
      "inspection carte 3"
    ]
...
const posteSoudureTranslations = {
  ...
  fr: {
    title: "Réparation postes soudure",
    captions: [
      "Réparation poste soudure 1",
      "Réparation poste soudure 2",
      "Réparation poste soudure 3",
      "Réparation poste soudure 4",
      "Réparation poste soudure 5"
    ]
  }
};
  ...
  const serviceCaptions = document.querySelectorAll('.services-today .service-caption');
  serviceCaptions.forEach((cap, idx) => { cap.textContent = serviceTranslations[lang].captions[idx]; });

  // فيديو اليوم
  document.querySelector('.videos-today h2').textContent = videoTranslations[lang].title;
  const videoCaptions = document.querySelectorAll('.videos-today .service-caption');
  videoCaptions.forEach((cap, idx) => { cap.textContent = videoTranslations[lang].captions[idx]; });

  // تصليح ماكينات لحام
  document.querySelector('#postesSection h2').textContent = posteSoudureTranslations[lang].title;
  const posteCaptions = document.querySelectorAll('#postesSection .service-caption');
  posteCaptions.forEach((cap, idx) => { cap.textContent = posteSoudureTranslations[lang].captions[idx]; });
}
...
// تنفيذها مباشرة بعد تحميل الصفحة
updatePrayerTimes();
