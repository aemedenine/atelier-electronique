...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function updateTime() {
    ...
    let day, month;
    if(currentLang==='ar'){day=daysAr[now.getDay()];month=monthsAr[now.getMonth()];} 
    else {day=daysFr[now.getDay()];month=monthsFr[now.getMonth()];}
    const date = now.getDate();
    const hours = now.getHours().toString().padStart(2,'0');
    const minutes = now.getMinutes().toString().padStart(2,'0');
    const seconds = now.getSeconds().toString().padStart(2,'0');
    timeEl.textContent = (currentLang==='ar' ? `${day}، ${date} ${month}` : `${day}, ${date} ${month}`) + ` - ${hours}:${minutes}:${seconds}`;
  }

  function updateVisits() {
    const key='aem-visit-count'; let count=parseInt(localStorage.getItem(key))||0; count++; localStorage.setItem(key,count);
  ...
  function updateNews(){const news=currentLang==='ar'?newsAr:newsFr; ticker.classList.remove('fade'); void ticker.offsetWidth; ticker.textContent=news[newsIndex]; ticker.classList.add('fade'); newsIndex=(newsIndex+1)%news.length;}
  ...
  radioBtn.addEventListener('click',()=>{if(radio.paused){radio.play().catch(e=>console.warn('Radio play failed:',e)); radioBtn.textContent=currentLang==='ar'?'أوقف الراديو':'Arrêter la radio';} else{radio.pause(); radioBtn.textContent=currentLang==='ar'?'شغّل الراديو':'Écouter la radio';} updateEqualizerVisibility();});
  radio.addEventListener('play',updateEqualizerVisibility); radio.addEventListener('pause',updateEqualizerVisibility);
  ...
  function setLanguage(lang){
    ...
    startNewsRotation(); updateTime(); updateVisits(); initFAQ();
  }

  toggleBtn.addEventListener('click',()=>{setLanguage(currentLang==='ar'?'fr':'ar');});
  setInterval(updateTime,1000);
  updateTime(); updateVisits(); startNewsRotation(); initFAQ(); updateEqualizerVisibility();
  ...
  function updateStars(rating){ratingStars.forEach((star,idx)=>{star.textContent=idx<rating?'★':'☆';}); averageDisplay.textContent=`تقييمك: ${rating} / 5`;}
  updateStars(savedRating);
  ratingStars.forEach((star,idx)=>{star.addEventListener('click',()=>{tempRating=idx+1; updateStars(tempRating);});});
  confirmBtn.addEventListener('click',()=>{savedRating=tempRating; localStorage.setItem('workshopRating',savedRating); updateStars(savedRating); alert('تم حفظ تقييمك!⭐');});
});
