...
document.addEventListener('DOMContentLoaded', () => {
   ...
   // ── Rating Stars - شخصي + عام (متوسط مخزن في Firebase) ────────────────
...
// مرجع Firebase للتقييمات
const ratingsRef = firebase.database().ref('ratings');

// تحميل المتوسط العام في الوقت الفعلي
ratingsRef.on('value', snapshot => {
    const data = snapshot.val() || { sum: 0, count: 0 };
    const average = data.count > 0 ? (data.sum / data.count).toFixed(1) : 0;
    
    if (avgStarsEl) avgStarsEl.textContent = average;
    if (voteCountEl) voteCountEl.textContent = data.count;
});

// تحديث النجوم (شخصي + عام)
...
// تحميل التقييم الشخصي عند التحميل
...
stars.forEach(star => {
    ...
    star.addEventListener('click', () => {
        // حفظ التقييم الشخصي محلياً
        selectedRating = val;
        localStorage.setItem('workshopRating', selectedRating);
        updateStars(selectedRating);

        // حفظ في Firebase (زيادة المجموع + العدد)
        ratingsRef.transaction(current => {
            const data = current || { sum: 0, count: 0 };
            // نمنع التقييم المتكرر من نفس المستخدم (اختياري)
            // إذا تحب نمنع واحد يقيم أكثر من مرة، نزيد check بـ user UID
            data.sum += val;
            data.count += 1;
            return data;
        });
