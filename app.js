...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // ── Daily Featured Items Rotation (الجزء المهم) ────────────────────────────────
    function setDailyItems() {
        const now = new Date();
        ...
        console.log("Daily rotation - Day of year:", dayOfYear);

        // خدمات اليوم
        const serviceCandidates = document.querySelectorAll('#servicesSlider .daily-candidate');
        console.log("خدمات مرشحة:", serviceCandidates.length);
        if (serviceCandidates.length > 0) {
            const index = dayOfYear % serviceCandidates.length;
            console.log("مختار خدمة index:", index);
            document.getElementById('daily-service').innerHTML = serviceCandidates[index].innerHTML;
        }

        // تصليح ماكينات لحام اليوم
        const repairCandidates = document.querySelectorAll('#postesSection .daily-candidate');
        console.log("تصليح مرشحة:", repairCandidates.length);
        if (repairCandidates.length > 0) {
            const index = dayOfYear % repairCandidates.length;
            console.log("مختار تصليح index:", index);
            document.getElementById('daily-repair').innerHTML = repairCandidates[index].innerHTML;
        }

        // فيديو اليوم
        const videoCandidates = document.querySelectorAll('#videoSlider .daily-candidate');
        console.log("فيديوهات مرشحة:", videoCandidates.length);
        if (videoCandidates.length > 0) {
            const index = dayOfYear % videoCandidates.length;
            console.log("مختار فيديو index:", index);
            const container = document.getElementById('daily-video');
            container.innerHTML = videoCandidates[index].innerHTML;

            // إعادة تفعيل hover play/pause للفيديو الجديد
            const video = container.querySelector('video');
            if (video) {
                video.muted = true;
                video.loop = true;
                video.addEventListener('mouseenter', () => video.play().catch(e => console.log("Video play error:", e)));
                video.addEventListener('mouseleave', () => {
                    video.pause();
                    video.currentTime = 0;
                });
